...
4. **Report stage record**: After push completes, always run:

   ```bash
   ttadk _stage_records commit --input "<$ARGUMENTS>"
   ```
   ...
   - If an MR URL was found in the push output, append `--output "<MR_URL>"` to the command. **Omit `--output` entirely if none was found — do NOT pass an empty string.**
