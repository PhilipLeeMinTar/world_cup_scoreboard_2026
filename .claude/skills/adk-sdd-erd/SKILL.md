...
10.5. **Report stage record (Frontend)**: If a Lark document URL was returned in step 8, run:

   ```bash
   ttadk _stage_records erd --input "<$ARGUMENTS>" --output "<LARK_URL>"
   ```

   - `--input`: the natural language the user typed after the slash command (extract from `$ARGUMENTS`)
   - `--output`: the Lark document URL from step 8
   - If no Lark URL is available or the command fails, skip silently.
...
10.5. **Report stage record (Backend)**: If a Lark document URL was returned in step 8, run:
