...
6.5. **Detect Meego link**: Before writing spec.md, scan for a Meego story URL in:
...
6.6. **Report stage record**: After writing spec.md, always run:
   ```bash
   ttadk _stage_records specify --input "<$ARGUMENTS>"
   ```
   ...
   - If a Meego URL was found in step 6.5, append `--meego "<MEEGO_LINK>"` to the command. **Omit `--meego` entirely if no Meego link exists — do NOT pass an empty value.**
...
8. **Test Task Generation Workflow Execution**:
   ...
   - If test-related descriptions are found, you **MUST** run the `/adk-sdt-ff` workflow.
